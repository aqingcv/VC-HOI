...
class DataFactory(Dataset):
    def __init__(self, name, partition, data_root, clip_model_name, zero_shot=False, zs_type='rare_first', num_classes=600): ##ViT-B/16, ViT-L/14@336px
        ...
        if name == 'hicodet':
            ...
            image_dir = dict(
                train='v-coco/images/train2014',
                val='v-coco/images/train2014',
                trainval = 'v-coco/images/train2014',
                test='v-coco/images/val2014'
            )
            self.dataset = VCOCO(
        ...
        if name == 'hicodet' and partition == 'train2015':
            self.text_feates_folder = os.path.join(data_root, 'hico_20160224_det/qwenvl_img_caption', 'train_caption_roberta128.npy')
        if name == 'hicodet' and partition == 'test2015':
            self.text_feates_folder = os.path.join(data_root, 'hico_20160224_det/qwenvl_img_caption', 'test_caption_roberta128.npy')
        # if name == 'hicodet' and partition == 'train2015':
        #     self.text_feates_folder = os.path.join(data_root, 'hico_20160224_det/Instructblip_img_caption', 'train_caption_roberta128.npy')
        # if name == 'hicodet' and partition == 'test2015':
        #     self.text_feates_folder = os.path.join(data_root, 'hico_20160224_det/Instructblip_img_caption', 'test_caption_roberta128.npy')
        if name == 'vcoco' and partition == 'trainval':  
            self.text_feates_folder = os.path.join(data_root, 'v-coco/qwenvl_img_caption', 'train_caption_roberta128.npy')
        if name == 'vcoco' and partition == 'test':    
            self.text_feates_folder = os.path.join(data_root, 'v-coco/qwenvl_img_caption', 'test_caption_roberta128.npy')
        self.text_feates_data = np.load(self.text_feates_folder, mmap_mode='r')  

        if name == 'hicodet' and partition == 'train2015':
            self.pose_folder = os.path.join(data_root, 'hico_20160224_det/pose_train', 'predictions')
        if name == 'hicodet' and partition == 'test2015':
            self.pose_folder = os.path.join(data_root, 'hico_20160224_det/pose_test', 'predictions')
        if name == 'vcoco' and partition == 'trainval':
            self.pose_folder = os.path.join(data_root, 'v-coco/pose_train', 'predictions')
        if name == 'vcoco' and partition == 'test':
            self.pose_folder = os.path.join(data_root, 'v-coco/pose_test', 'predictions')
    ...
    def __getitem__(self, i):
        ...
        text_feates = self.text_feates_data[self.keep[i]]
        image_text = torch.tensor(text_feates)
        file_index = filename[:-4] + str('.json')
        pose_file_path = os.path.join(self.pose_folder, file_index)

        with open(pose_file_path, 'r') as f:
            pose_data = json.load(f)
        keypoints = [data['keypoints'] for data in pose_data]
        image_pose = torch.as_tensor(keypoints, dtype=torch.float32)

        boxes_pose = [data['bbox'] for data in pose_data]
        pose_boxes = torch.as_tensor(boxes_pose, dtype=torch.float32).reshape(-1, 4)
        # (image, target), filename = self.dataset[i]
        if self.name == 'hicodet' and self.zero_shot and self.partition == 'train2015':
        ...
        return (image,image_clip,image_text,image_pose,pose_boxes), target
