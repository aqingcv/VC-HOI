...
class Adapter(nn.Module):
    def __init__(self,
        ...
        instance_decoder_layer = TransformerDecoderLayer(self.down_size, 8, self.down_size*4,
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, adapter: bool=False, adapter_num_layers: int=1):
        ...
        if adapter:
            self.adaptermlp = Adapter(None,  d_model=d_model , dropout=0.1, bottleneck=128,
                                    init_option='lora',
...
class CLIP(nn.Module):
    ...
    def text_to_embedding(self, text):
        """ text ([List[Tensor]): A list of action text tokens.
            [
                [action text 1],
                [action text 2],
                ...
        ...
        eot_indices = []
        for action_token in text:
            remain_length = self.context_length - self.prefix_length - self.conjun_length - len(action_token)
            if remain_length < 0:
            ...
            token = torch.cat([action_token, padding_zeros])
...
def build_model(state_dict: dict, use_adapter=True, adapter_pos='all', adapter_num_layers=1, prefix_length=8, conjun_length=4):
    ...
    model = CLIP(
        ...
        adapter_layers=adapter_layers, adapter_num_layers=adapter_num_layers, prefix_length=prefix_length, conjun_length=conjun_length
    )
