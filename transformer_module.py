...
class TransformerCrossDecoderLayer(nn.Module):
    ...
    def forward_pre(self, tgt, memory, sup_memory=None,
                    tgt_mask: Optional[Tensor] = None,
                    memory_mask: Optional[Tensor] = None,
                    tgt_key_padding_mask: Optional[Tensor] = None,
                    memory_key_padding_mask: Optional[Tensor] = None,
        ...
        tgt3 = self.norm3(tgt)
        tgt3 = self.multihead_attn(query=tgt3, key=sup_memory,
                                   value=sup_memory, attn_mask=memory_mask,
                                   key_padding_mask=memory_key_padding_mask)[0]
        tgt = tgt + self.dropout3(tgt3)
    ...
    def forward(self, tgt, memory, sup_memory=None,
                ...
                query_pos: Optional[Tensor] = None):
        if self.normalize_before:
            return self.forward_pre(tgt, memory, sup_memory, tgt_mask, memory_mask,
                                    tgt_key_padding_mask, memory_key_padding_mask, pos, query_pos)
